...
class CollectionOrder(models.Model):
    ...
    _inherit = ["loan.basic.model", 'company.field.mixin']
    _table = "C_order"
    _description = "催收订单"

    order_status_id = fields.Many2one("collection.order.status", string=_("Collection Order Status"), index=True)
    collection_status = fields.Char(string=_("Collection Status"), related="order_status_id.code")
    loan_order_id = fields.Many2one("loan.order", string=_("Associated Orders"), index=True)
    company_id = fields.Many2one('res.company', related="loan_order_id.company_id", store=True, index=True)
    ...
    loan_uid = fields.Char(_('UserID'), related='loan_order_id.loan_uid')
    ...
    is_overdue = fields.Boolean('是否逾期', related="loan_order_id.is_overdue")
    can_derate_amount = fields.Float('可减免金额', related="loan_order_id.can_derate_amount")
    ...
    is_relative = fields.Char(string=_("Relative Number"), compute="_compute_from_user_address_book")
    is_risk = fields.Char(string=_("Risk Number"), compute="_compute_from_user_address_book")
    ...
    def _compute_version(self):
        for rec in self:
            version = self.env["loan.app.version"].search([('app_id', '=', rec.app_id.id)], order='version_code desc', limit=1)
            rec.version =  version.version_code if version else 0
    ...
    @api.depends("user_address_book_ids.name")
    def _compute_from_user_address_book(self):
        total = "数量" if self.env.user.lang == "zh_CN" else "Total"
        for rec in self:
            people = rec.user_address_book_ids
            rec.is_relative = f"aunt、uncle、friend; {total}: {len(people.filtered(lambda c: c.is_relative))}"
            rec.is_risk = f"bank、loan; {total}: {len(people.filtered(lambda c: c.is_risk))}"
    ...
    def _compute_loan_orders(self):
        for rec in self:
            rec.loan_order_ids = rec.user_id.loan_order_ids.filtered(lambda x: x.id != rec.loan_order_id.id and x.order_status in ["7", "8"])
    ...
    def action_server_collection_order_allot(self):
        ...
        return {
            ...
            "target": "current",
            "domain": [("id", "in", filtered_ids), ('order_status_id.code', '=', '1'),('id', '=', 0)],
    ...
    def action_server_collection_order_pending(self):
        ...
        return {
            ...
            "search_view_id": [search_view_id.id],
            "target": "current",
            "domain": [("id", "in", filtered_ids), ('order_status_id.code', '=', '2')],
            "context": dict(context),
        }
    ...
    def action_server_collection_order_process(self):
        ...
        domain = [("order_status_id", "=", order_status_id.id), ("collector_id", "=", self.env.user.id)]
        ...
        return {
            ...
            "search_view_id": [search_view_id.id],
            "target": "current",
            "domain": [("id", "in", filtered_ids), ('order_status_id.code', '=', '3')],
            "context": dict(context),
        }
...
class CollectionUserContact(models.Model):
    ...
    collection_order_id = fields.Many2one("collection.order", string=_("Collection Order"), index=True, ondelete="cascade")
...
class CollectionUserAddressBook(models.Model):
    ...
    def _compute_from_name(self):
        for rec in self:
            name = rec.name.lower() if isinstance(rec.name, str) else ""
...
class HistoryCollectionRecord(models.Model):
    ...
    collection_order_id = fields.Many2one("collection.order", string=_("Collection Order"), index=True, ondelete="cascade")
    sequence = fields.Integer(string=_("Serial Number"), compute="_compute_sequence", store=True)
    user_id = fields.Many2one("collection.user.contact", string=_("Collection Target"), domain="[('collection_order_id', '=', collection_order_id)]")
    relation_selection = fields.Selection(
        [
            ("Father", "Father"),
            ("Mother", "Mother"),
    ...
    collector_id = fields.Many2one("res.users", string=_("Collector"), default=lambda self: self.env.user.id)
    collection_stage = fields.Char(
        string=_("Collection Stage"),
